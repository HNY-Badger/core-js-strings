...
function countVowels(str) {
  ...
  let sum = 0;
  for (let i = 0; i < lowCaseStr.length; i += 1) {
    sum += vowels.includes(lowCaseStr[i]) ? 1 : 0;
  }
  return sum;
}
...
function isPalindrome(str) {
  ...
  const specSymb = ['!', '?'];
  lowCaseStr = lowCaseStr.replace(/\s/g, ''); // Remove whhite spaces
  lowCaseStr = lowCaseStr.replace(/,/g, ''); // Remove commas
  specSymb.forEach((symb) => {
    lowCaseStr = lowCaseStr.replace(symb, '');
  });

  const len = lowCaseStr.length - 1;
  for (let i = 0; i <= len; i += 1) {
    if (lowCaseStr[i] !== lowCaseStr[len - i]) return false;
  }
  return true;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    sentence = sentence.replace(words[i], rev);
  }
  return sentence;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (charCode >= 65 && charCode <= 90) {
      inverse += str.charAt(i).toLowerCase();
    } else if (charCode >= 97 && charCode <= 122) {
      inverse += str.charAt(i).toUpperCase();
    } else {
      inverse += String.fromCharCode(charCode);
    }
  }
  return inverse;
}
